...
from .utils import get_audio_features, get_rays, get_bg_coords, convert_poses, get_mulitdim_feat
...
class NeRFDataset_Test:
    def __init__(self, opt, device, downscale=1):
        ...
        for f in tqdm.tqdm(frames, desc=f'Loading data'):
            ...
            if self.opt.exp_eye:
                ...
                area = -area * math.log2(area) - (1-area) * math.log2(1-area)
        ...
        if self.opt.asr:
            ...
            # auds is novel, may have a different length with images
                ...
                self.auds = aud_features[0:self.end_index]
                print ('[INFO] aud len===', len(self.auds))
        self.bg_img = torch.from_numpy(self.bg_img)

        self.time = np.array(self.time, dtype=np.float32)
        self.time = torch.from_numpy(self.time).view(-1, 1)
        if self.opt.exp_eye:
            self.eye_area = np.array(self.eye_area, dtype=np.float32) # [N]
            self.eye_area = self.eye_area * ((self.eye_area - self.eye_area.min()) / (self.eye_area.max() - self.eye_area.min()))
    ...
    def collate(self, index):
        ...
        if self.opt.exp_eye:
            results['eye'] = get_mulitdim_feat(self.eye_area, index[0], self.opt.dim_eye).to(self.device)
            results['time'] = get_mulitdim_feat(self.time, index[0], self.opt.dim_eye).to(self.device) # [1]
        else:
            results['eye'] = None

        bg_img = self.bg_img.view(1, -1, 3).repeat(B, 1, 1).to(self.device)

        results['bg_color'] = None#bg_img
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1):
        ...
        # only load one specified split
            ...
            _split = 'val' if type == 'test' else type  #_split = 'val'
        ...
        if type == 'train': #false
            ...
            frames = frames[:7] # first 7 frames for val
        elif type == 'test':
            frames = frames[0:300]    
        ...
        if not self.opt.asr:
            ...
            # cross-driven extracted features. 
                ...
                aud_features = np.load(self.opt.aud)
                # test 30 seconds aud
                aud_features = aud_features[0:250] 
            aud_features = torch.from_numpy(aud_features)
        ...
        for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
            if self.opt.srtask is False:
                ...
                #f_path = os.path.join(self.root_path, 'com_imgs', str(f['img_id']) + '.jpg')   ### tensor included in sr stage
                f_path = os.path.join(self.root_path, 'gt_imgs', str(f['img_id']) + '.jpg')
            ...
            pose = nerf_matrix_to_ngp(pose, scale=self.scale, offset=self.offset)
            self.poses.append(pose)
            e_path = os.path.join(self.root_path, 'eye_features', str(f['img_id']) + '.pt')
            eye_feature = torch.load(e_path)
            self.eye_features.append(eye_feature)
            ...
            if self.opt.srtask:
                hr_path = os.path.join(self.root_path, 'hr_imgs', str(f['img_id']) + '.jpg')
                self.hr_imgs.append(hr_path)
            # load frame-wise bg
            ...
            # find the corresponding audio to the image frame
            if not self.opt.asr and self.opt.aud == '': #true
                aud = aud_features[min(f['aud_id'], aud_features.shape[0] - 1)] # careful for the last frame... [29, 16]
                self.auds.append(aud)
            ...
            if 'face_rect' not  in f:
                ...
                ymin, xmin= rect[0], rect[1]
                ymax ,xmax = ymin + rect[2], xmin + rect[3]
                self.face_total.append([xmin, xmax, ymin, ymax])
            else:
                ymin, xmin= f['face_rect'][0], f['face_rect'][1]
                ymax ,xmax = ymin + f['face_rect'][2], xmin + f['face_rect'][3]
               
                self.face_total.append([xmin, xmax, ymin, ymax])
            
            fxmin, fxmax, fymin, fymax = xmin, xmax, ymin, ymax
            ...
            if self.opt.exp_eye:
                ...
                area = -area * math.log2(area) - (1-area) * math.log2(1-area)
                self.eye_area.append(area)
                # up_left conner of left and right eye
                self.eye_co.append([int(lms[eyes_left, 1].min()),int(lms[eyes_left, 0].min()),int(lms[eyes_right, 1].min()),int(lms[eyes_right, 0].min()),
                                    int(lms[eyes_left, 1].max()),int(lms[eyes_left, 0].max()),int(lms[eyes_right, 1].max()),int(lms[eyes_right, 0].max())])   
        ...
        if self.preload > 0:
            ...
            if self.opt.srtask:
                self.hr_imgs = np.array(self.hr_imgs)
        if self.opt.asr:
            # live streaming, no pre-calculated auds
            self.auds = None
        else:
        ...
        self.eye_features = torch.stack(self.eye_features, dim = 0)
        ...
        if self.opt.exp_eye:
            ...
            if self.opt.smooth_eye: #false when training
    ...
    def collate(self, index):
        ...
        if self.auds is not None:
            auds = get_audio_features(self.auds, self.opt.att, index[0]).to(self.device)
            results['auds'] = auds
        results['eye_feature'] = self.eye_features[index[0]].to(self.device)
        # head pose and bg image may mirror (replay --> <-- --> <--).
        index[0] = self.mirror_index(index[0])
        ...
        if self.training and self.opt.finetune_lips:
            lip_rect = self.lips_rect[index[0]]
            face_total = self.face_total[index[0]]
            results['rect'] = lip_rect
            rays = get_rays(poses, self.intrinsics, self.H, self.W, -1, lip_rect=lip_rect, face_rect = face_total)
        else:
            face_total = self.face_total[index[0]]
            #add lip
            lip_rect = self.lips_rect[index[0]]
            eye_co = self.eye_co[index[0]]
            rays = get_rays(poses, self.intrinsics, self.H, self.W, self.num_rays, self.opt.patch_size,lip_rect=lip_rect,face_rect = face_total,eye_co = eye_co)
        ...
        if self.training:
            ...
            lx_min, ly_min, rx_min, ry_min, lx_max, ly_max, rx_max, ry_max = \
                self.eye_co[index[0]]
            patch_left_x, patch_left_y = int((lx_min + lx_max)/2 - self.opt.patch_size/2), int((ly_min + ly_max)/2 - self.opt.patch_size/2)
            patch_right_x, patch_right_y = int((rx_min + rx_max)/2 - self.opt.patch_size/2), int((ry_min + ry_max)/2 - self.opt.patch_size/2)
            eye_mask = ((rays['j']>=patch_left_x) & (rays['j']<patch_left_x+self.opt.patch_size) & (rays['i']>=patch_left_y) & (rays['i']<patch_left_y+self.opt.patch_size)) | \
                ((rays['j']>=patch_right_x) & (rays['j']<patch_right_x+self.opt.patch_size) & (rays['i']>=patch_right_y) & (rays['i']<patch_right_y+self.opt.patch_size))
        ...
        if self.opt.exp_eye:
            ...
            results['eye'] = None
        results['time'] = get_mulitdim_feat(self.time, index[0], self.opt.dim_eye).to(self.device)
        # load bg
        ...
        if self.preload == 0: # on the fly loading
            ...
            if bg_torso_img.shape[0] != self.H or bg_torso_img.shape[1] != self.W:
                bg_torso_img = cv2.resize(bg_torso_img, (self.W, self.H), interpolation=cv2.INTER_AREA)
        ...
        C = images.shape[-1]
        images_patch = torch.gather(images.view(B, -1, C), 1, torch.stack(C * [rays['inds']], -1)) # [B, N, 3/4]
        ...
        if self.opt.srtask:
            hr_imgs = self.hr_imgs[index]
            hr_imgs = cv2.imread(hr_imgs[0], cv2.IMREAD_UNCHANGED) # [H, W, 3]
            hr_imgs = cv2.cvtColor(hr_imgs, cv2.COLOR_BGR2RGB)
            hr_imgs = hr_imgs.astype(np.float32) / 255 # [H, W, 3]
            hr_imgs = torch.from_numpy(hr_imgs).unsqueeze(0)
            hr_imgs = hr_imgs.to(self.device) 
            if self.training:
                C = hr_imgs.shape[-1]
                hr_imgs = torch.gather(hr_imgs.view(B, -1, C), 1, torch.stack(C * [rays['inds_hr']], -1))  #[B, N, 3/4]
            results['hr_imgs'] = hr_imgs
