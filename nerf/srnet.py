...
def make_layer(basic_block, num_basic_block, **kwarg):
    ...
    Args:
        ...
        num_basic_block (int): number of blocks. #5
...
class RRDB_SFT(nn.Module):
    ...
    Used in RRDB-Net in ESRGAN.
    Args:
        num_feat (int): Channel number of intermediate features. #64
        num_grow_ch (int): Channels for each growth. #32
    """
...
class SFTNet(nn.Module):
    ...
    def __init__(self, n_in_colors, scale, num_feat=64, num_block=5, num_grow_ch=32, num_cond=1, dswise=False):
        ...
        if dswise: #false
            ...
            self.conv_first = nn.Conv2d(n_in_colors, num_feat, 3, 1, 1) #n_in_colors 3, num_feat 64,
...
class UNetDiscriminatorSN(nn.Module):
    ...
    def forward(self, x):
        # print(f' ori size: {x.shape[2]}, {x.shape[3]}')
        x = F.interpolate(x, size=(x.shape[2] // 8 * 8, x.shape[3] // 8 * 8), mode='bilinear', align_corners=False)
        # print(f' scale size: {x.shape[2]}, {x.shape[3]}')
