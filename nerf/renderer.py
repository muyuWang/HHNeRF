...
import raymarching
from .utils import custom_meshgrid, get_audio_features, euler_angles_to_matrix, convert_poses, get_mulitdim_feat
...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, auds, bg_coords, poses, eye=None, time=None, eye_feature=None,index=0, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1  [1, 65536, 3]
        # auds: [B, 16]
        # index: [B]
        ...
        ## use trainable resnet to extract eye features
        #eye_feature = self.extractor(eye_feature)
        ##
        ...
        if self.training:
            ...
            sigmas, rgbs, ambient = self(xyzs, dirs, enc_a, ind_code, eye, time,eye_feature)
            sigmas = self.density_scale * sigmas
            #print('training shapes',xyzs.shape,rays.shape,sigmas.shape,ambient.shape,rays_o.shape)
            ...
            #print ('ambient_sum==',sigmas.shape,ambient.shape,ambient_sum.shape)
            # for training only
            #print('training ambient',ambient_sum.shape,ambient.abs().sum(-1).shape,ambient)
            ...
            ambient_sum = torch.zeros(N, dtype=dtype, device=device)
            ...
            while step < max_steps:
                ...
                #print('xyzs shape',xyzs.shape,rays_o.shape)
                sigmas, rgbs, ambient = self(xyzs, dirs, enc_a, ind_code, eye, time,eye_feature, True)
                #ambient_sum.append(ambient.abs().sum(-1))
                #print ('ambient' ,ambient)
                #print('ambient==',rgbs.shape,ambient.shape,ambient.abs().sum(-1).shape)
                sigmas = self.density_scale * sigmas
                raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, ambient.abs().sum(-1),deltas, weights_sum, ambient_sum, depth, image, T_thresh)
                #weights_sum, ambient_sum, depth, image = raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, weights_sum, depth, image, T_thresh)
            ...
            results['ambient'] = ambient_sum
            #results['ambient'] = ambient_sum
            #print('result ambient shape',results['ambient'].shape)
        # background
        if bg_color is None:
            bg_color = 1
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        if not self.torso: # forbid updating head if is training torso...
            ...
            if self.exp_eye:
                eye = get_mulitdim_feat(self.eye_area, rand_idx, self.opt.dim_eye).to(self.density_bitfield.device)
                # eye = self.eye_area[[rand_idx]].to(self.density_bitfield.device) # [1, 1]
            else:
                eye = None
            time = get_mulitdim_feat(self.time, rand_idx, self.opt.dim_eye).to(self.density_bitfield.device)
            #time = self.time[[rand_idx]].to(self.density_bitfield.device)
            # full update
            X = torch.arange(self.grid_size, dtype=torch.int32, device=self.density_bitfield.device).split(S)
            Y = torch.arange(self.grid_size, dtype=torch.int32, device=self.density_bitfield.device).split(S)
            Z = torch.arange(self.grid_size, dtype=torch.int32, device=self.density_bitfield.device).split(S)
                ...
                for ys in Y:
                    for zs in Z:
                        ...
                        for cas in range(self.cascade):
                            ...
                            # query density
                            sigmas = self.density(cas_xyzs, enc_a, eye, time)['sigma'].reshape(-1).detach().to(tmp_grid.dtype)
